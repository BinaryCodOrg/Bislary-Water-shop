  ...
  async function actionHandler(heading, row, action) {
    ...
    switch (action) {
      case "edit":
        setInitialValues({
          ...row,
          amount: toCommaAndPKR(parseFloat(row.amount)),
        });
        break;
      case "delete":
        break;
    ...
    <Formik
      ...
      onSubmit={async (values, { resetForm }) => {
        try {
          ...
          if (!initialValues?.id) {
            result = await window.api.addExpense(payload); //Smart Order Add
          } else {
            result = await window.api.updateExpense(payload); //Smart Order Add
          }
